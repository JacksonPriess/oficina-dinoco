...
/*
    ...
    void deveCriarClientePFComSucesso() {
        ...
        when(repository.save(any(ClienteEntity.class))).thenReturn(clienteMockSalvo);
        // Act
    ...
    void deveLancarExcecaoQuandoDocumentoJaEstiverCadastrado() {
        ...
        assertEquals("ClienteEntity já cadastrado com este documento.", exception.getMessage());
        verify(repository, never()).save(any(ClienteEntity.class));
    }
    ...
    void deveBuscarPorIdComSucesso() {
        Long id = 1L;
        ClienteEntity clienteExistente = ClienteBuilder.umClientePF().comId(id).comNome("Maria").build();
    ...
    @Test
    @DisplayName("Deve buscar entidade ClienteEntity por ID com sucesso")
    void deveBuscarEntidadePorIdComSucesso() {
        Long id = 1L;
        ClienteEntity clienteExistente = ClienteBuilder.umClientePJ().comId(id).build();
        when(repository.findById(id)).thenReturn(Optional.of(clienteExistente));
        ClienteEntity resultado = service.buscarEntidadePorId(id);
    ...
    @Test
    @DisplayName("Deve lançar exceção ao buscar ClienteEntity por ID inexistente")
    ...
    void deveAtualizarClienteComSucesso() {
        // Arrange
        Long id = 1L;
        ClienteEntity clienteExistente = ClienteBuilder.umClientePF()
                .comId(id)
                .comNome("Nome Antigo")
        ...
        when(repository.findById(id)).thenReturn(Optional.of(clienteExistente));
        when(repository.save(any(ClienteEntity.class))).thenReturn(clienteExistente);
        // Act
        service.atualizar(id, requestAtualizacao);
        // Assert
        verify(repository).save(clienteCaptor.capture());
        ClienteEntity clienteAtualizado = clienteCaptor.getValue();
    ...
    void deveLancarExcecaoAoAtualizarDocumento() {
        Long id = 1L;
        ClienteEntity clienteExistente = ClienteBuilder.umClientePF().comDocumento("11111111111").build();
    ...
    void deveLancarExcecaoAoAtualizarTipoPessoa() {
        Long id = 1L;
        // ClienteEntity base é PF
        ClienteEntity clienteExistente = ClienteBuilder.umClientePF().comTipoPessoa("F").build();
    ...
    void deveDesativarClienteComSucesso() {
        Long id = 1L;
        ClienteEntity cliente = ClienteBuilder.umClientePF().comId(id).build();
        when(repository.findById(id)).thenReturn(Optional.of(cliente));
        service.desativar(id);
        verify(repository).save(clienteCaptor.capture());
        ClienteEntity clienteInativado = clienteCaptor.getValue();
